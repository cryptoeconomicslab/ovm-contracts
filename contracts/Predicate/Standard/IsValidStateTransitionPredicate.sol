...
contract IsValidStateTransitionPredicate is AtomicPredicate {
        ...
        returns (bool)
    {
        ...
        require(
            transaction.predicateAddress == txAddress,
            "transaction.predicateAddress must be Tx.address"
        );
        require(
        ...
        require(
            isContainedPredicate.decide(inputsForIsContained),
            "range must be included"
        );
        require(
            keccak256(transaction.inputs[2]) ==
                keccak256(previousStateUpdate.inputs[2]),
            "input block number must be same"
        );
        require(
            keccak256(transaction.inputs[3]) ==
                keccak256(stateUpdate.inputs[3]),
            "state object must be same"
        );
        return true;
    }
